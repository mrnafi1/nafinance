...
export default function App() {
  ...
  const deleteTx = tx => {
    setConfirmDialog({
      show: true, msg: settings.lang === 'bn' ? "মুছতে চান?" : "Delete transaction?", onConfirm: () => {
      ...
      <main className="animate-slide" style={{ maxWidth: 480, margin: "0 auto", padding: "15px 20px 140px", position: "relative", zIndex: 1 }}>
        ...
        {tab === "planning" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 30 }}>
            ...
            {/* 🔥 নতুন চ্যালেঞ্জ ভিউ */}
            <ChallengesView data={data} setData={setData} setConfirmDialog={setConfirmDialog} fmt={fmt} TH={TH} lang={settings.lang} showToast={showToast} />
          </div>
        )}
                ...
                {data.savings.history.map(sv => (
                  <div key={sv.id} className="tx-card glass-panel" style={{ padding: "16px 20px", borderRadius: 20, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <button onClick={() => {
                        setConfirmDialog({
                          show: true, msg: settings.lang === 'bn' ? "এই হিস্ট্রি মুছবেন?" : "Delete this record?", onConfirm: () => {
...
function GraphsView({ data, fmt, TH, lang, getCategories }) {
  ...
  useEffect(() => { setSelectedDay(null); }, [gType]);
  ...
  const heatmapData = useMemo(() => {
    ...
    (data.txs || []).filter(t =>
      t.type === 'expense' &&
      t.category?.toLowerCase() !== 'transfer' // ক্যাপিটাল/স্মল যাই হোক, এখন ফিল্টার হবে
    ).forEach(t => {
    ...
    for (let i = 59; i >= 0; i--) {
      ...
      days.push({ date: dateStr, value: expByDate[dateStr] || 0 });
    }
    return days;
  }, [data.txs]);
