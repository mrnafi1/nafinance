...
import {
  BarChart, Bar, PieChart, Pie, Cell, Tooltip, ResponsiveContainer, XAxis, Legend
} from "recharts";
import {
  Plus, Trash2, Home, BarChart2, Settings, Users, X, Download, Eye, EyeOff, Search, 
  AlertTriangle, Landmark, Wallet, Lock, Sun, Moon, KeyRound, Edit3, Code, Check, FileText, Edit
} from "lucide-react";
...
export default function App() {
  const [data, setData] = useState(() => {
    try { const saved = localStorage.getItem("nafinance_vpro_db"); return saved ? { ...DEFAULT_DATA, ...JSON.parse(saved) } : DEFAULT_DATA; } catch(e) { return DEFAULT_DATA; }
  });

  const [settings, setSettings] = useState(() => {
    try { const saved = localStorage.getItem("nafinance_vpro_set"); return saved ? { ...DEFAULT_SETTINGS, ...JSON.parse(saved) } : DEFAULT_SETTINGS; } catch(e) { return DEFAULT_SETTINGS; }
  });
...
function HomeView({ data, setData, fmt, TH, settings, setSettings, getCategories, deleteTx, setEditTxData, setModal, setConfirmDialog }) {
  ...
  const exportPDF = () => {
    ...
    data.txs.forEach(tx => {
      ...
      html += `<tr><td>${tx.date}</td><td>${cat.label.en}</td><td>${w.name}</td><td>${tx.note || '-'}</td><td class="${tx.type === 'income' ? 'inc' : 'exp'}">${tx.type.toUpperCase()}</td><td class="${tx.type === 'income' ? 'inc' : 'exp'}">${fmtMoney(tx.amount, settings.curr, 'en')}</td></tr>`;
    });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      {overdueDebts.map(d => (
        <div key={`od-${d.id}`} style={{ background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.3)", padding: "14px 18px", borderRadius: 18, display: "flex", alignItems: "center", gap: 12, color: "#ef4444" }}><AlertTriangle size={20}/> <span style={{fontSize:14, fontWeight:800}}>{settings.lang==='bn'?'ওভারডিউ ধার:':'Overdue Debt:'} {d.person} ({fmt(d.amount)})</span></div>
      ))}
      {activeAlerts.map(cat => (
        <div key={cat.id} style={{ background: "rgba(245,158,11,0.15)", border: "1px solid rgba(245,158,11,0.3)", padding: "14px 18px", borderRadius: 18, display: "flex", alignItems: "center", justifyContent: "space-between" }}><div style={{display:"flex", alignItems:"center", gap: 12}}><AlertTriangle size={20} color="#f59e0b"/> <span style={{fontSize:14, fontWeight:800, color:TH.text}}>{cat.icon} {cat.label[settings.lang] || cat.label['en']} {settings.lang==='bn'?'বাজেট ৮০% শেষ!':'80% budget used!'}</span></div><button onClick={() => setData({ ...data, dismissedAlerts: [...(data.dismissedAlerts || []), cat.id] })} style={{background:"none", border:"none", color: TH.textMid}}><X size={20}/></button></div>
      ))}
      ...
      <div style={{ padding: 30, borderRadius: 32, background: "linear-gradient(135deg, #1e1b4b, #0f172a)", color: "#fff", boxShadow: "0 20px 40px rgba(0,0,0,0.3)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}><div><p style={{ fontSize: 13, opacity: 0.7, fontWeight: 800 }}>{settings.lang==='bn'?'মোট ব্যালেন্স':'Total Balance'}</p><h2 style={{ fontSize: 44, fontWeight: 900, margin: "10px 0" }}>{fmt(total)}</h2></div><button onClick={()=>setSettings({...settings, hideBalance: !settings.hideBalance})} style={{ padding: 14, background: "rgba(255,255,255,0.1)", border: "none", borderRadius: 16, color: "#fff" }}>{settings.hideBalance ? <Eye size={22}/> : <EyeOff size={22}/>}</button></div>
        <div style={{ display: "flex", gap: 10, marginTop: 25 }}><div style={{ flex: 1 }}><p style={{ fontSize: 11, opacity: 0.6 }}>{settings.lang==='bn'?'এই মাসে আয়':'This Month Inc'}</p><p style={{ fontSize: 16, fontWeight: 800, color: "#10b981" }}>{fmt(monthlyInc)}</p></div><div style={{ flex: 1, borderLeft: "1px solid rgba(255,255,255,0.1)", paddingLeft: 10 }}><p style={{ fontSize: 11, opacity: 0.6 }}>{settings.lang==='bn'?'এই মাসে ব্যয়':'This Month Exp'}</p><p style={{ fontSize: 16, fontWeight: 800, color: "#ef4444" }}>{fmt(monthlyExp)}</p></div><div style={{ flex: 1, borderLeft: "1px solid rgba(255,255,255,0.1)", paddingLeft: 10 }}><p style={{ fontSize: 11, opacity: 0.6 }}>{settings.lang==='bn'?'সঞ্চয়':'Savings'}</p><p style={{ fontSize: 16, fontWeight: 800, color: "#3b82f6" }}>{fmt(monthlySav)}</p></div></div>
      </div>

      <div style={{ display: "flex", gap: 10 }}><button onClick={exportCSV} style={{ flex:1, padding:"12px", borderRadius:18, background:TH.bgInner, color:TH.text, fontWeight:800, display:"flex", alignItems:"center", justifyContent:"center", gap:8, border:"none" }}><Download size={18} color="#10b981"/> CSV</button><button onClick={exportPDF} style={{ flex:1, padding:"12px", borderRadius:18, background:TH.bgInner, color:TH.text, fontWeight:800, display:"flex", alignItems:"center", justifyContent:"center", gap:8, border:"none" }}><FileText size={18} color="#ef4444"/> PDF</button><select value={dateRange} onChange={e=>setDateRange(e.target.value)} style={{ padding: "0 15px", borderRadius: 18, background: TH.bgInner, border: "none", color: TH.text, fontWeight: 800, outline:"none", flex:1.5 }}><option value="all">{settings.lang==='bn'?'সব সময়':'All Time'}</option><option value="month">{settings.lang==='bn'?'এই মাস':'This Month'}</option><option value="week">{settings.lang==='bn'?'গত ৭ দিন':'Last 7 Days'}</option></select></div>
      <div style={{ display: "flex", gap: 10 }}><div style={{ flex: 2, display: "flex", alignItems: "center", gap: 12, background: TH.bgCard, padding: "14px 18px", borderRadius: 20, border: `1px solid ${TH.border}` }}><Search size={20} color={TH.textMid}/><input type="text" placeholder={settings.lang==='bn'?'খুঁজুন...':'Search...'} value={search} onChange={e=>setSearch(e.target.value)} style={{ background:"none", border:"none", color:TH.text, outline:"none", flex:1, fontSize:15, fontWeight:700 }}/></div><select value={filterCat} onChange={e=>setFilterCat(e.target.value)} style={{ flex: 1, padding: "0 15px", borderRadius: 20, background: TH.bgCard, border: `1px solid ${TH.border}`, color: TH.text, fontWeight: 800, outline:"none", fontSize:14 }}><option value="all">{settings.lang==='bn'?'ক্যাটাগরি':'Category'}</option>{getCategories("expense").map(c => <option key={c.id} value={c.id}>{c.icon} {c.label[settings.lang] || c.label['en']}</option>)}</select></div>
        ...
        {filteredTxs.slice(0, 30).map(tx => {
          ...
          return (
            <div key={tx.id} style={{ padding: 20, background: TH.bgCard, borderRadius: 25, border: `1px solid ${TH.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }} onClick={() => { setEditTxData(tx); setModal("tx"); }}>
              <div style={{ display: "flex", alignItems: "center", gap: 16 }}><div style={{ width: 54, height: 54, borderRadius: 18, background: cat.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26 }}>{cat.icon}</div><div><p style={{ fontWeight: 800, fontSize: 17, color: TH.text }}>{tx.note || cat.label[settings.lang] || cat.label['en']}</p><p style={{ fontSize: 12, color: TH.textMid, fontWeight:600 }}>{tx.date}</p></div></div>
              <div style={{ display: "flex", alignItems: "center", gap: 15 }}><p style={{ fontWeight: 900, fontSize: 18, color: tx.type === "income" ? "#10b981" : "#ef4444" }}>{tx.type === "income" ? "+" : "-"}{fmt(tx.amount)}</p><button onClick={(e)=>{e.stopPropagation(); deleteTx(tx);}} style={{ color: "#ef4444", background: "rgba(239,68,68,0.1)", border: "none", padding: 12, borderRadius: 14 }}><Trash2 size={18}/></button></div>
            </div>
          );
        })}
      </div>
...
function AssetsView({ data, setData, fmt, TH, showToast, settings, setConfirmDialog }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}><h3 style={{ fontWeight: 800, fontSize:18 }}>{settings.lang==='bn'?'ওয়ালেট':'Wallets'}</h3><button onClick={()=>setWalletForm({...walletForm, show:true})} style={{ padding: "10px 18px", borderRadius: 14, background: "#3b82f6", color: "#fff", border: "none", fontWeight: 800 }}>+ Add</button></div>
      {walletForm.show && (
         <div style={{ padding: 25, background: TH.bgCard, borderRadius: 25, display: "flex", flexDirection: "column", gap: 14, border: "2px dashed #3b82f6" }}><div style={{ display:"flex", gap:12 }}><input type="text" placeholder="Icon" value={walletForm.icon} onChange={e=>setWalletForm({...walletForm, icon: e.target.value})} style={{ width:65, padding: 16, borderRadius: 16, background: TH.bgInner, border: "none", color: TH.text, textAlign:"center", outline:"none" }} /><input type="text" placeholder="Wallet Name" value={walletForm.name} onChange={e=>setWalletForm({...walletForm, name: e.target.value})} style={{ flex:1, padding: 16, borderRadius: 16, background: TH.bgInner, border: "none", color: TH.text, outline:"none", fontWeight:700 }} /></div><input type="number" placeholder="Initial Balance" value={walletForm.balance} onChange={e=>setWalletForm({...walletForm, balance: e.target.value})} style={{ padding: 16, borderRadius: 16, background: TH.bgInner, border: "none", color: TH.text, outline:"none", fontWeight:700 }} /><div style={{ display:"flex", gap:12 }}><button onClick={handleWalletSave} style={{ flex:1, padding: 16, background: "#3b82f6", color: "#fff", border: "none", borderRadius: 16, fontWeight: 900 }}>Save</button><button onClick={()=>setWalletForm({ show: false, name: "", balance: "", icon: "💳", id: null })} style={{ flex:1, padding: 16, background: TH.bgInner, color: TH.text, border: "none", borderRadius: 16, fontWeight: 900 }}>Cancel</button></div></div>
      )}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        {data.wallets.map(w => (
          <div key={w.id} style={{ padding: 25, background: TH.bgCard, borderRadius: 30, border: `1px solid ${TH.border}`, position:"relative" }}><div style={{ position:"absolute", top:18, right:18, display:"flex", gap:10 }}><button onClick={()=>setWalletForm({show:true, ...w})} style={{background:"none", border:"none", color:"#3b82f6"}}><Edit size={18}/></button><button onClick={()=>deleteWalletSafe(w)} style={{background:"none", border:"none", color:"#ef4444"}}><Trash2 size={18}/></button></div><span style={{ fontSize: 36 }}>{w.icon}</span><p style={{ fontSize: 14, fontWeight: 800, color: TH.textMid, marginTop: 12 }}>{w.name}</p><p style={{ fontSize: 24, fontWeight: 900 }}>{fmt(w.balance)}</p></div>
        ))}
      </div>
      ...
      {debtForm.show && (
          <div style={{ padding: 25, background: TH.bgCard, borderRadius: 30, display: "flex", flexDirection: "column", gap: 14, border: "2px dashed #8b5cf6" }}><input type="text" placeholder={settings.lang==='bn'?'নাম':'Name'} value={debtForm.person} onChange={e=>setDebtForm({...debtForm, person: e.target.value})} style={{ padding: 16, borderRadius: 16, background: TH.bgInner, border: "none", color: TH.text, fontWeight: 700, outline:"none" }} /><input type="number" placeholder={settings.lang==='bn'?'টাকা':'Amount'} value={debtForm.amount} onChange={e=>setDebtForm({...debtForm, amount: e.target.value})} style={{ padding: 16, borderRadius: 16, background: TH.bgInner, border: "none", color: TH.text, fontWeight: 700, outline:"none" }} /><label style={{fontSize:13, fontWeight:800, color:TH.textMid}}>{settings.lang==='bn'?'লেনদেনের তারিখ:':'Given Date:'}</label><input type="date" value={debtForm.date} onChange={e=>setDebtForm({...debtForm, date: e.target.value})} style={{ padding: 16, borderRadius: 16, background: TH.bgInner, border: "none", color: TH.text, outline:"none", fontWeight:700 }} /><label style={{fontSize:13, fontWeight:800, color:TH.textMid}}>{settings.lang==='bn'?'ফেরত পাওয়ার তারিখ:':'Return Date:'}</label><input type="date" value={debtForm.returnDate} onChange={e=>setDebtForm({...debtForm, returnDate: e.target.value})} style={{ padding: 16, borderRadius: 16, background: TH.bgInner, border: "none", color: TH.text, outline:"none", fontWeight:700 }} /><select value={debtForm.sourceId} onChange={e=>setDebtForm({...debtForm, sourceId: e.target.value})} style={{ padding: 16, borderRadius: 16, background: TH.bgInner, border: "none", color: TH.text, fontWeight: 800, outline:"none" }}>{data.wallets.map(w=><option key={w.id} value={w.id}>{w.icon} {w.name}</option>)}</select><select value={debtForm.type} onChange={e=>setDebtForm({...debtForm, type: e.target.value})} style={{ padding: 16, borderRadius: 16, background: TH.bgInner, border: "none", color: TH.text, fontWeight: 800, outline:"none" }}><option value="lend">{settings.lang==='bn'?'আমি পাবো':'I will get'}</option><option value="borrow">{settings.lang==='bn'?'আমি দেবো':'I will give'}</option></select><button onClick={handleAddDebt} style={{ padding: 18, background: "#8b5cf6", color: "#fff", border: "none", borderRadius: 16, fontWeight: 900, fontSize:16 }}>Save</button></div>
      )}
      {data.debts.map(d => (
...
function PlanningView({ data, setData, fmt, TH, settings, getCategories, showToast, setConfirmDialog }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div style={{ display: "flex", background: TH.bgCard, padding: 8, borderRadius: 22, border: `1px solid ${TH.border}` }}>{['vault', 'goals', 'budgets'].map(t => (<button key={t} onClick={()=>setSubTab(t)} style={{ flex: 1, padding: "14px", borderRadius: 16, background: subTab===t ? "#8b5cf6" : "transparent", color: subTab===t ? "#fff" : TH.textMid, fontWeight: 800, border: "none", fontSize: 14 }}>{t.toUpperCase()}</button>))}</div>
      ...
      {subTab === "goals" && (
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}><button onClick={()=>setGoalForm({...goalForm, show: true})} style={{ padding:20, background:TH.bgCard, borderRadius:25, border:`2px dashed #8b5cf6`, color:TH.text, fontWeight:800, fontSize:16 }}>+ {settings.lang==='bn'?'নতুন লক্ষ্য':'New Goal'}</button>{goalForm.show && (<div style={{ padding:25, background:TH.bgCard, borderRadius:30, display:"flex", flexDirection:"column", gap:14, border:"1.5px solid #8b5cf6" }}><input type="text" placeholder={settings.lang==='bn'?'নাম':'Name'} value={goalForm.name} onChange={e=>setGoalForm({...goalForm, name: e.target.value})} style={{ padding:16, borderRadius:16, background:TH.bgInner, color:TH.text, border:"none", outline:"none", fontWeight:700 }} /><input type="number" placeholder={settings.lang==='bn'?'টার্গেট':'Target Amount'} value={goalForm.target} onChange={e=>setGoalForm({...goalForm, target: e.target.value})} style={{ padding:16, borderRadius:16, background:TH.bgInner, color:TH.text, border:"none", outline:"none", fontWeight:700 }} /><input type="text" placeholder={settings.lang==='bn'?'নোট':'Note'} value={goalForm.note} onChange={e=>setGoalForm({...goalForm, note: e.target.value})} style={{ padding:16, borderRadius:16, background:TH.bgInner, color:TH.text, border:"none", outline:"none", fontWeight:700 }} /><button onClick={()=>{ if(!goalForm.name || !goalForm.target) return showToast("Enter info", "error"); const newGoal = { id: goalForm.id || genId(), name: goalForm.name, target: Number(goalForm.target), note: goalForm.note, saved: goalForm.saved || 0 }; setData({ ...data, goals: goalForm.id ? data.goals.map(g=>g.id===goalForm.id?newGoal:g) : [newGoal, ...data.goals] }); setGoalForm({ show: false, name: "", target: "", note: "", id: null }); }} style={{ padding:18, background:"#8b5cf6", color:"#fff", border:"none", borderRadius:16, fontWeight:900, fontSize:16 }}>Save Goal</button></div>)}{data.goals.map(g => (<div key={g.id} style={{ padding:28, background:TH.bgCard, borderRadius:30, border:`1px solid ${TH.border}` }}><div style={{ display:"flex", justifyContent:"space-between", marginBottom:15 }}><h4 style={{ fontWeight:800, fontSize:18 }}>🎯 {g.name}</h4><div style={{ display:"flex", gap:12 }}><button onClick={()=>setGoalForm({show:true, ...g})} style={{ color:"#3b82f6", background:"none", border:"none" }}><Edit3 size={20}/></button><button onClick={()=>setConfirmDialog({show:true, msg:"Delete Goal?", onConfirm:()=>setData({...data, goals: data.goals.filter(x=>x.id!==g.id)})})} style={{ color:"#ef4444", background:"none", border:"none" }}><Trash2 size={20}/></button></div></div><p style={{ fontSize: 13, color: TH.textMid, marginBottom: 12, fontWeight:600 }}>{g.note}</p><div style={{ height:14, background:TH.bgInner, borderRadius:10, overflow:"hidden", marginBottom:12 }}><div style={{ width:`${Math.min((g.saved/g.target)*100, 100)}%`, height:"100%", background:"linear-gradient(90deg, #3b82f6, #8b5cf6)", transition:"width 1s ease" }} /></div><div style={{ display:"flex", justifyContent:"space-between", fontSize:14, fontWeight:800, marginBottom:18 }}><span style={{color:"#10b981"}}>{fmt(g.saved)}</span><span style={{color:TH.textMid}}>{fmt(g.target)}</span></div>{addCashId === g.id ? (<div style={{ display:"flex", gap:10 }}><select value={addCashWallet} onChange={e=>setAddCashWallet(e.target.value)} style={{ padding:14, borderRadius:14, background:TH.bgInner, border:"none", color:TH.text, outline:"none", flex:1, fontWeight:800 }}>{data.wallets.map(w=><option key={w.id} value={w.id}>{w.name}</option>)}</select><input type="number" placeholder="Amt" value={addCashAmt} onChange={e=>setAddCashAmt(e.target.value)} style={{ flex:1, padding:14, borderRadius:14, background:TH.bgInner, border:"none", color:TH.text, outline:"none", fontWeight:800 }}/><button onClick={()=>handleAddGoalCash(g.id)} style={{ padding:14, borderRadius:14, background:"#10b981", color:"#fff", border:"none", fontWeight:800 }}><Check size={20}/></button><button onClick={()=>{setAddCashId(null); setAddCashAmt("");}} style={{ padding:14, borderRadius:14, background:"rgba(239,68,68,0.15)", color:"#ef4444", border:"none", fontWeight:800 }}><X size={20}/></button></div>) : (<button onClick={()=>setAddCashId(g.id)} style={{ width:"100%", padding:16, borderRadius:16, background:TH.bgInner, color:TH.text, fontWeight:800, border:"none", fontSize:15 }}>Add Cash +</button>)}</div>))}</div>
      )}
      
      {subTab === "budgets" && (
        <div style={{ display:"flex", flexDirection:"column", gap:18 }}><div style={{ padding:28, background:TH.bgCard, borderRadius:30, border:`1.5px solid #8b5cf6` }}><p style={{ fontWeight:800, marginBottom:15, color:"#8b5cf6", fontSize:15 }}>{settings.lang==='bn'?'বাজেট সেট করুন':'Set Budget'}</p><div style={{ display:"flex", gap:12, marginBottom:15 }}><select value={budgetCat} onChange={e=>setBudgetCat(e.target.value)} style={{ flex:1, padding:16, borderRadius:16, background:TH.bgInner, color:TH.text, border:"none", fontWeight:800, outline:"none", fontSize:15 }}><option value="">{settings.lang==='bn'?'ক্যাটাগরি':'Category'}</option>{getCategories("expense").map(c => <option key={c.id} value={c.id}>{c.icon} {c.label[settings.lang] || c.label['en']}</option>)}</select><input type="number" placeholder="Limit" value={limitVal} onChange={e=>setLimitVal(e.target.value)} style={{ width:120, padding:16, borderRadius:16, background:TH.bgInner, color:TH.text, border:"none", textAlign:"center", fontWeight:900, outline:"none", fontSize:16 }} /></div><button onClick={()=>{ const val=Number(limitVal); if(budgetCat && val){ setData({...data, budgets:{...data.budgets, [budgetCat]:val}}); setLimitVal(""); showToast("Updated", "success"); } }} style={{ width:"100%", padding:18, borderRadius:16, background:"#8b5cf6", color:"#fff", border:"none", fontWeight:900, fontSize:16 }}>Update Budget</button></div>{Object.entries(data.budgets).map(([id, lim]) => { const cat = getCategories("expense").find(c => c.id === id); if(!cat) return null; const spent = data.txs.filter(x => x.type === "expense" && x.category === id && x.date.startsWith(TODAY().slice(0, 7))).reduce((s, e) => s + e.amount, 0); const percent = Math.min((spent / lim) * 100, 100); const isOver = spent > lim; return (<div key={id} style={{ padding:25, background:TH.bgCard, borderRadius:28, border:`1px solid ${isOver ? '#ef4444' : TH.border}`, display:"flex", flexDirection:"column", gap:12 }}><div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}><div style={{ display:"flex", alignItems:"center", gap:16 }}><span style={{ fontSize:32 }}>{cat.icon}</span><div><p style={{ fontWeight:800, fontSize:16 }}>{cat.label[settings.lang] || cat.label['en']}</p><p style={{ fontSize:12, color:TH.textMid, fontWeight:700 }}>{fmt(spent)} / {fmt(lim)}</p></div></div><button onClick={()=>setConfirmDialog({show:true, msg:"Delete Budget?", onConfirm:() => { const n={...data.budgets}; delete n[id]; setData({...data, budgets:n}); }})} style={{ color:"#ef4444", background:"none", border:"none" }}><Trash2 size={20}/></button></div><div style={{ height:10, background:TH.bgInner, borderRadius:10, overflow:"hidden", marginTop:5 }}><div style={{ width:`${percent}%`, height:"100%", background: isOver ? "#ef4444" : "#8b5cf6", transition:"width 1s ease" }} /></div></div>); })}</div>
      )}
    </div>
  );
}
...
function GraphsView({ data, fmt, TH, lang, getCategories }) {
  ...
  const weeklyData = useMemo(() => Array.from({length: 7}).map((_, i) => { const d = new Date(); d.setDate(d.getDate() - (6 - i)); const s = d.toISOString().split('T')[0]; const inc = data.txs.filter(t => t.type === "income" && t.date === s).reduce((sum, t) => sum + t.amount, 0); const exp = data.txs.filter(t => t.type === "expense" && t.date === s).reduce((sum, t) => sum + t.amount, 0); return { name: d.toLocaleDateString(lang==='bn'?'bn-BD':'en-US', {weekday:'short'}), income: inc, expense: exp }; }), [data.txs, lang]);
  const monthlyData = useMemo(() => ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"].map((m, i) => { const y = new Date().getFullYear(); const mStr = `${y}-${String(i+1).padStart(2, '0')}`; const inc = data.txs.filter(t => t.type === "income" && t.date.startsWith(mStr)).reduce((s, t) => s + t.amount, 0); const exp = data.txs.filter(t => t.type === "expense" && t.date.startsWith(mStr)).reduce((s, t) => s + t.amount, 0); return { name: m, income: inc, expense: exp }; }), [data.txs]);
  return (<div style={{ display: "flex", flexDirection: "column", gap: 24 }}><div style={{ display: "flex", background: TH.bgCard, padding: 8, borderRadius: 25, border: `1px solid ${TH.border}` }}>{['breakdown', 'weekly', 'monthly'].map(t => (<button key={t} onClick={()=>setGType(t)} style={{ flex: 1, padding: "14px", borderRadius: 18, background: gType===t ? "rgba(139,92,246,0.15)" : "transparent", color: gType===t ? "#8b5cf6" : TH.textMid, fontWeight: 800, border: "none", fontSize: 13 }}>{t.toUpperCase()}</button>))}</div><div style={{ padding: 30, background: TH.bgCard, borderRadius: 40, border: `1px solid ${TH.border}`, minHeight: 420 }}>{gType === "breakdown" ? (<ResponsiveContainer width="100%" height={380}><PieChart><Pie data={getCategories("expense").map(cat => ({ name: cat.label[lang] || cat.label['en'], value: data.txs.filter(x=>x.type==="expense" && x.category===cat.id).reduce((s,e)=>s+e.amount,0), color: cat.color })).filter(x=>x.value>0)} innerRadius={90} outerRadius={120} paddingAngle={8} dataKey="value" stroke="none">{getCategories("expense").map((e,i)=><Cell key={i} fill={e.color}/>)}</Pie><Tooltip formatter={v=>fmt(v)} contentStyle={{borderRadius: 20, border:"none", background: TH.bgInner, fontWeight:800}}/></PieChart></ResponsiveContainer>) : (<ResponsiveContainer width="100%" height={380}><BarChart data={gType === "weekly" ? weeklyData : monthlyData} margin={{top:20}}><XAxis dataKey="name" stroke={TH.textMid} fontSize={12} tickLine={false} axisLine={false}/><Tooltip cursor={{fill: 'rgba(255,255,255,0.05)'}} formatter={v=>fmt(v)} contentStyle={{borderRadius: 18, border:"none", background: TH.bgInner, fontWeight:800}}/><Legend verticalAlign="top" height={40} iconType="circle"/><Bar dataKey="income" fill="#10b981" radius={[6,6,0,0]} name={lang==='bn'?'আয়':'Income'} barSize={15} /><Bar dataKey="expense" fill="#ef4444" radius={[6,6,0,0]} name={lang==='bn'?'ব্যয়':'Expense'} barSize={15} /></BarChart></ResponsiveContainer>)}</div></div>);
}
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 2000, display: "flex", alignItems: "flex-end", justifyContent: "center", padding: 10, backdropFilter: "blur(12px)" }}>
      <div style={{ background: TH.bgCard, padding: "35px 30px", borderRadius: "45px 45px 25px 25px", width: "100%", maxWidth: 480, border: `1px solid ${TH.border}`, animation: "slideIn 0.3s ease-out" }}>
        ...
        {!editData && (<label style={{ display:"flex", alignItems:"center", gap:12, marginBottom:25, fontWeight:800, color:TH.textMid, cursor:"pointer", padding:"10px", background:TH.bgInner, borderRadius:15 }}><input type="checkbox" checked={isRecurring} onChange={e=>setIsRecurring(e.target.checked)} style={{width:20, height:20, accentColor:"#8b5cf6"}}/>{lang==='bn'?'প্রতি মাসে স্বয়ংক্রিয়ভাবে যোগ করুন':'Auto-add every month'}</label>)}
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 2000, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
      <div style={{ background: TH.bgCard, padding: "40px 30px 60px", borderRadius: "45px 45px 0 0", width: "100%", maxWidth: 480, maxHeight: "94vh", overflowY: "auto", borderTop:`1px solid ${TH.border}` }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 15 }}><button onClick={()=>setSettings({...settings, theme: "light"})} style={{ padding: 18, borderRadius: 18, background: settings.theme==='light'?'#f59e0b':TH.bgInner, color: settings.theme==='light'?'#fff':TH.text, border: "none", fontWeight:800 }}><Sun size={18}/> Light</button><button onClick={()=>setSettings({...settings, theme: "dark"})} style={{ padding: 18, borderRadius: 18, background: settings.theme==='dark'?'#8b5cf6':TH.bgInner, color: settings.theme==='dark'?'#fff':TH.text, border: "none", fontWeight:800 }}><Moon size={18}/> Dark</button></div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}><button onClick={()=>setSettings({...settings, lang: "bn"})} style={{ padding: 18, borderRadius: 18, background: settings.lang==='bn'?'#8b5cf6':TH.bgInner, color: settings.lang==='bn'?'#fff':TH.text, border: "none", fontWeight:800 }}>বাংলা</button><button onClick={()=>setSettings({...settings, lang: "en"})} style={{ padding: 18, borderRadius: 18, background: settings.lang==='en'?'#8b5cf6':TH.bgInner, color: settings.lang==='en'?'#fff':TH.text, border: "none", fontWeight:800 }}>English</button></div>
        ...
        <div style={{ padding:22, background:TH.bgInner, borderRadius:25, marginBottom:25 }}><p style={{ fontWeight:800, marginBottom:12, color:"#8b5cf6", fontSize:15 }}>{settings.lang==='bn'?'নতুন ক্যাটাগরি':'Add Category'}</p><div style={{ display:"flex", gap:10, marginBottom:12 }}><select value={newCat.type} onChange={e=>setNewCat({...newCat, type:e.target.value})} style={{ padding:14, borderRadius:14, background:TH.bgCard, color:TH.text, border:"none", outline:"none", fontWeight:700 }}><option value="expense">{settings.lang==='bn'?'ব্যয়':'Expense'}</option><option value="income">{settings.lang==='bn'?'আয়':'Income'}</option></select><input type="text" placeholder="🍔" value={newCat.icon} onChange={e=>setNewCat({...newCat, icon:e.target.value})} style={{ width:60, padding:14, borderRadius:14, background:TH.bgCard, color:TH.text, border:"none", textAlign:"center", outline:"none" }} /><input type="color" value={newCat.color} onChange={e=>setNewCat({...newCat, color:e.target.value})} style={{ width:50, height:48, padding:0, border:"none", borderRadius:12, background:"none", cursor:"pointer" }} /></div><input type="text" placeholder={settings.lang==='bn'?'নাম':'Name'} value={newCat.name} onChange={e=>setNewCat({...newCat, name:e.target.value})} style={{ width:"100%", padding:16, borderRadius:14, background:TH.bgCard, color:TH.text, border:"none", marginBottom:14, outline:"none", fontWeight:700 }} /><button onClick={()=>{ if(!newCat.name) return showToast("Enter Name", "error"); const n = { id: genId(), label: { bn: newCat.name, en: newCat.name }, icon: newCat.icon, color: newCat.color, bg: `${newCat.color}20` }; setData({ ...data, customCategories: { ...data.customCategories, [newCat.type]: [...(data.customCategories[newCat.type] || []), n] } }); setNewCat({ type: "expense", name: "", icon: "📦", color: "#8b5cf6" }); showToast("Added!", "success"); }} style={{ width:"100%", padding:16, background:"#8b5cf6", color:"#fff", border:"none", borderRadius:14, fontWeight:900, fontSize:15 }}>Save Category</button></div>
        <div style={{ padding:22, background:TH.bgInner, borderRadius:25, marginBottom:25 }}><p style={{ fontWeight:800, marginBottom:12, color:"#8b5cf6", fontSize:15 }}>PIN SECURITY</p><div style={{ display:"flex", gap:10, marginBottom:12 }}><input type="number" placeholder="4-Digit" value={newPin} onChange={e=>setNewPin(e.target.value.slice(0,4))} style={{ flex:1, padding:15, borderRadius:14, background:TH.bgCard, color:TH.text, border:"none", textAlign:"center", fontWeight:800, outline:"none" }} /><input type="text" placeholder="Secret Word" value={recovery} onChange={e=>setRecovery(e.target.value)} style={{ flex:1, padding:15, borderRadius:14, background:TH.bgCard, color:TH.text, border:"none", textAlign:"center", fontWeight:800, outline:"none" }} /></div><button onClick={()=>{ if(newPin.length===4 && recovery){ setSettings({...settings, pinLock: newPin, recoveryWord: recovery.toLowerCase()}); showToast("PIN Set", "success"); } }} style={{ width:"100%", padding:16, background:"#10b981", color:"#fff", border:"none", borderRadius:14, fontWeight:900 }}>Set PIN</button>{settings.pinLock && <button onClick={()=>setSettings({...settings, pinLock:"", recoveryWord:""})} style={{ width:"100%", marginTop:12, color:"#ef4444", fontWeight:800, background:"none", border:"none" }}>Remove PIN</button>}</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 25 }}><button onClick={()=>{ const blob = new Blob([JSON.stringify({data, settings})], {type: "application/json"}); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `NaFinance_Backup.json`; link.click(); showToast("Backup Success", "success"); }} style={{ padding: 18, borderRadius: 16, background: TH.bgInner, color: TH.text, fontWeight: 800, border:"none" }}>Backup Data</button><label style={{ padding: 18, borderRadius: 16, background: TH.bgInner, color: TH.text, fontWeight: 800, textAlign:"center" }}>Restore <input type="file" style={{display:"none"}} onChange={(e)=>{ const reader = new FileReader(); reader.onload = (ev) => { try { const p = JSON.parse(ev.target.result); if(p.data) setData({...DEFAULT_DATA, ...p.data}); if(p.settings) setSettings({...DEFAULT_SETTINGS, ...p.settings}); showToast("Restore Success", "success"); } catch(err) { showToast("Invalid File", "error"); } }; reader.readAsText(e.target.files[0]); }}/></label></div>
        <button onClick={()=>{ setConfirmDialog({show:true, msg:settings.lang==='bn'?"সব মুছে যাবে! নিশ্চিত?":"Are you sure to Reset?", onConfirm:()=>{localStorage.clear(); window.location.reload();}}) }} style={{ width: "100%", padding: 20, background: "rgba(239,68,68,0.15)", color: "#ef4444", border: "none", borderRadius: 20, fontWeight: 900, fontSize:16 }}>Factory Reset</button>
      </div>
    </div>
  );
}
